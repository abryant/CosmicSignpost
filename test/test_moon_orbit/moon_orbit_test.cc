...
TEST(MoonOrbit, MoonIcrfAtJ2000) {
  ...
  EXPECT_NEAR(moonFromEarth.position.getX(), -291'608'384.1877129, 200'000);
  EXPECT_NEAR(moonFromEarth.position.getY(), -274'979'741.6731504, 200'000);
  EXPECT_NEAR(moonFromEarth.position.getZ(), 36'271'196.62699287, 200'000);
...
TEST(MoonOrbit, EarthMoonBarycenterIcrfAtJ2000) {
  ...
  // Vector from JPL Horizons:
  // https://ssd.jpl.nasa.gov/horizons/app.html
  EXPECT_NEAR(earthMoonBaryCenter.position.getX(), -3'543'212.282604189, 3000);
  EXPECT_NEAR(earthMoonBaryCenter.position.getY(), -3'341'164.558342924, 3000);
  EXPECT_NEAR(earthMoonBaryCenter.position.getZ(), 440'716.2357539921, 3000);
